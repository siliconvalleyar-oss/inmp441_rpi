  ...
  -d, --duration <SEC>       Recording duration in seconds (default: 5, min 1).
      ...
      --gain <DB>            Digital gain applied to recordings (default 0).
       ...
       --lpf <HZ>             Low-pass filter cutoff applied after the HPF
                              (default 0 Hz; 0 = off). Attenuates high-
                              frequency noise and the INMP441's ultrasonic
                              response. Use values like 8000 or 12000 to
                              reduce hiss.
...
output format WAV/MP3, the high-pass cutoff (option 8), the low-pass cutoff
(option 9), a 5-second level test, and record.
...
- **Sample rate, channel, stereo, duration, warmup, gain, high-pass cutoff
  (`hpf_hz`), low-pass cutoff (`lpf_hz`), dropout threshold, meter interval,
  menu format (WAV/MP3) and Bluetooth MAC** (`bt_mac`).
